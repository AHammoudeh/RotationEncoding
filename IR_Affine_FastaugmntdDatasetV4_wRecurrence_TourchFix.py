...
registration_method = 'matching_points' #{'1block', 'matching_points', 'recurrent_matrix'}
imposed_point = 0

Arch = 'ResNet'#{'VTX', 'U-Net','ResNet', 'DINO' , }
if registration_method == 'recurrent_matrix':
    recurrence_type = 'Additive'
    recurrent_schedule = 'random_aff_param'#'completely_random' #'scheduledrandom' #zero
else:
    recurrent_schedule = ''
...
if registration_method == 'matching_points':
    ...
    if imposed_point==0:
        ...
        recurrent_schedule = str(imposed_point)
else:
...
file_savingfolder = '/home/ahmadh/MIR_savedmodel/Fixed_{}_{}_{}_{}_M0:{}/'.format(
            DATASET_generation, registration_method,Arch,Noise_level_dataset, recurrent_schedule)
...
def wrap_imge(Affine_mtrx, source_img, acc= 2*crop_ratio):
    #edited version on 20240301
    Affine_mtrx = workaround_matrix(Affine_mtrx, acc = acc)
...
def mtrx3(Affine_mtrx):
    AM3 = torch.zeros( [Affine_mtrx.shape[0],3,3]).to(device)
    AM3[:,0:2,:] = Affine_mtrx
    AM3[:,2,2] = 1
    return AM3
...
def pass_augment_img(image0, measure,NOISE_LEVEL=0, MODE='nearest'):
    ...
    wrapped_img = wrap_imge(Affine_mtrx, image0)
    return wrapped_img, Affine_mtrx, Affine_parameters
...
class Dataset(torch.utils.data.Dataset):
  def __init__(self, list_paths, dim = 128, batch_size = 25, DATASET='active', registration_method='recurrent_matrix', mode='train', recurrent_schedule = 'random'):
        ...
        self.mode = mode
        self.recurrent_schedule = recurrent_schedule
...
train_set = Dataset(list_paths=train_routes_source,  batch_size=batch_size, DATASET=DATASET_generation,registration_method =registration_method,recurrent_schedule = recurrent_schedule, mode='train')
trainloader = torch.utils.data.DataLoader(train_set, batch_size=batch_size,  shuffle=False) #num_workers=4,
#-------------
val_set = Dataset(list_paths=val_routes_source, batch_size=batch_size, DATASET=DATASET_generation, registration_method =registration_method,recurrent_schedule = recurrent_schedule,  mode='valid')
valloader = torch.utils.data.DataLoader(val_set, batch_size=batch_size,shuffle=True) #,num_workers=4
#-------------
...
class Build_IRmodel_Resnet(nn.Module):
    def __init__(self, resnet_model, registration_method = 'affine matrix'):
...
if Arch == 'ResNet':
    ...
    IR_Model = Build_IRmodel_Resnet(core_model, registration_method)
...
Affine_mtrx = pred['Affine_mtrx']
wrapped_img = wrap_imge(Affine_mtrx,X_batch['source'].to(device))
...
for EPOCH in range(TOTAL_Epochs):  # loop over the dataset multiple times
    i=-1
    running_loss = 0.0
    if recurrent_schedule == 'scheduledrandom':
    ...
    save_examples(IR_Model,valloader, n_examples = 6)
    if with_epch_loss:
...
test_set = Dataset(list_paths=test_routes_source, batch_size=batch_size, DATASET=DATASET_generation,
            registration_method =registration_method,recurrent_schedule = recurrent_schedule, mode='test')
...
class Build_IRmodel_AE(nn.Module):
    ...
    def forward(self, concatenated_input):
        ...
        if self.registration_method=='matching_points':
            ...
            Affine_mtrx = predicted_line.view(-1, 2, 3)
            predction = {'Affine_mtrx': Affine_mtrx}
        #wrapped_img = wrap_imge(Affine_mtrx, source_img)
        return predction
...
class Build_IRmodel_CNN(nn.Module):
    ...
    def forward(self, inputs):
      ...
      if self.registration_method=='1block':
       Affine_mtrx = predicted_line.view(-1, 2, 3)
