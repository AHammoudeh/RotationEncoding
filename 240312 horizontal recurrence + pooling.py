...
class Dataset(torch.utils.data.Dataset):
  def __init__(self, list_paths, dim = 128, batch_size = 25, DATASET={'train':'active','val':'','test':''},
                                    registration_method= 'Additive_Recurence', mode='train',
...
train_set = Dataset(list_paths=routes_source,  batch_size=batch_size, DATASET=DATASET_generation_split,
...
#-------------
val_set = Dataset(list_paths=routes_source, batch_size=batch_size, DATASET=DATASET_generation_split,
...
class Build_IRmodel_Resnet(nn.Module):
    ...
    def forward(self, input_X_batch):
        resnet_output = self.resnet_model(input_X_batch)
        flatened = torch.nn.Flatten()(resnet_output)
        predicted_line = self.fc3(flatened)
        Prd_Affine_mtrx = predicted_line.view(-1, 2, 3)
        predction = {'Affine_mtrx': Prd_Affine_mtrx}
        return predction

core_model = resnet18(pretrained=True)
core_model = torch.nn.Sequential(*list(core_model.children())[:-2])
IR_Model = Build_IRmodel_Resnet(core_model)

IR_Model(torch.zeros(1,3,128,256))


total_params = sum(p.numel() for p in IR_Model.parameters())
...
Maxpooling
class Build_IRmodel_Resnet(nn.Module):
    def __init__(self, resnet_model, registration_method = 'Rawblock', Glopooling = 'max'):
        super(Build_IRmodel_Resnet, self).__init__()
        self.resnet_model = resnet_model
        self.Glopooling = Glopooling
    ...
    def forward(self, input_X_batch):
        ...
        if 'Recurence' in self.registration_method:
            ...
            concatenated_input = torch.cat((source,target), dim=2)
        resnet_output = self.resnet_model(concatenated_input)
        print('1',resnet_output.shape)
        resnet_output1 = self.adapmaxpool(resnet_output)
        print('2',resnet_output1.shape)
        predicted_line = self.fc3(resnet_output1.squeeze())
        Prd_Affine_mtrx = predicted_line.view(-1, 2, 3)
        predction = {'Affine_mtrx': Prd_Affine_mtrx }
        return predction
...
m = IR_Model({'source':torch.zeros(1,3,128,128),'target':torch.zeros(1,3,128,128),'M_i':torch.zeros(2,3)})
'''
...
class Build_IRmodel_Resnet(nn.Module):
    def __init__(self, resnet_model, registration_method = 'Rawblock', Glopooling = 'avg', BW_Position=False):
        super(Build_IRmodel_Resnet, self).__init__()
        self.resnet_model = resnet_model
        self.BW_Position = BW_Position
...
if Arch == 'ResNet':
    ...
    if Glopooling == 'avg': #with_pooling:
        ...
        core_model.avgpool = Identity()
        core_model = torch.nn.Sequential(*list(core_model.children())[:-2])
    IR_Model = Build_IRmodel_Resnet(core_model, registration_method,Glopooling, BW_Position=False)
    core_model.to(device)
    IR_Model.to(device)
...
ext = '_bestVal'#'_EndTraining' #_bestVal
    ...
    IR_Model_tst = Build_IRmodel_Resnet(core_model_tst, registration_method, Glopooling)
...
test_set = Dataset(list_paths=routes_source, batch_size=batch_size, DATASET=DATASET_generation_split,
...
def Generate_Mi_batch(batch_size= batch_size, folder_suffix='completely_random', mode='test',
                        noise=0.5,Affine_mtrx=torch.zeros([batch_size,2,3]) ):
    if 'completely_random' in folder_suffix :
        M_i = torch.normal(torch.zeros([batch_size,2,3]), torch.ones([batch_size,2,3]))
    elif 'random_aff_param' in folder_suffix:
        M_i = torch.zeros_like(Affine_mtrx)
        for k in range(batch_size):
            M_i[k,:,:] =  Generate_random_AffineMatrix(measure=Intitial_Tx, NOISE_LEVEL=noise, device='cpu')[0][0]
    elif 'mix' in folder_suffix:
        if mode == 'train':
...
class Build_Ensemble_IRmodel(nn.Module):
    def __init__(self, model, Num_iterations=10, key = 'Affine_mtrx'):
    ...
    def forward(self, input_X_batch):
        predction= {}
        input_X_batch['M_i'] = Generate_Mi_batch(batch_size= batch_size, folder_suffix='completely_random', mode='test',
                        noise=0.5,Affine_mtrx=torch.zeros([batch_size,2,3]) ).to(device)
        predction[self.key] = self.model(input_X_batch)[self.key]
        for i in range(1,self.Num_iterations):
            input_X_batch['M_i'] = Generate_Mi_batch(batch_size= batch_size, folder_suffix='completely_random', mode='test',
                        noise=0.5,Affine_mtrx=torch.zeros([batch_size,2,3]) ).to(device)
            #torch.zeros_like(input_X_batch['M_i'])
            predction[self.key] += self.model(input_X_batch)[self.key].detach()
        predction[self.key]/= self.Num_iterations
        return predction

'''
Ensemble_model = Build_Ensemble_IRmodel(IR_Model_tst, Num_iterations=1)
Ensemble_model.eval()
MSE_affine_Ensemble = test_loss(Ensemble_model, testloader,200, key = 'Affine_mtrx').detach().item()
print(MSE_affine_Ensemble)
'''

ensemble_loss ={}
for n_models in [1, 3, 10, 20]:
    Ensemble_model = Build_Ensemble_IRmodel(IR_Model_tst, Num_iterations=n_models)
    Ensemble_model.eval()
...
def Ensemble_stat(model, inputs, Num_iterations=10, Num_batches=20 , key = 'Affine_mtrx'):
    predections= {}
    batch_size = int(inputs['source'].shape[0])
    with torch.no_grad():
        predections['all'] = torch.zeros(Num_iterations,Num_batches*batch_size,2,3)
        for j in range(Num_batches):
            for i in range(Num_iterations):
                inputs['M_i'] = Generate_Mi_batch(batch_size= batch_size, folder_suffix='completely_random',
                                        mode='test', noise=0.5,Affine_mtrx=torch.zeros([batch_size,2,3]) ).to(device)
                prd = model(inputs)
                predections['all'][i,int(j*batch_size):int((j+1)*batch_size),:,:] = prd[key]
            predections['avg'] = torch.mean(predections['all'], dim=[0])
            predections['var'] = torch.var(predections['all'], dim=[0])
    return predections



Prdction ={}
for n_models in [3, 10, 20]:
inputs
Prdction[str(n_models)]= Ensemble_stat(IR_Model_tst, inputs, Num_iterations=10, Num_batches=20 , key = 'Affine_mtrx')

e = Ensemble_stat(IR_Model_tst, x, 10, 5)

torch.stack(, , dim=3)
torch.avg()

predections[key],predections[key]
